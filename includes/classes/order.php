...
class order extends base
{
    ...
    public function query($order_id): void
    {
        ...
        $precision = zen_config('QUANTITY_DECIMALS') > 0 ? (int)zen_config('QUANTITY_DECIMALS') : 0;
...
//            'cc_cvv' => $GLOBALS['cc_cvv'] ?? '',
    ...
    public function create(array $zf_ot_modules): int|null
    {
        ...
        if ($this->info['total'] == 0) {
            if ((int)zen_config('DEFAULT_ZERO_BALANCE_ORDERS_STATUS_ID') === ([0-9])) {
    ...
    $GLOBALS[$payment_class]->email_footer !== '') ? nl2br($GLOBALS[$payment_class]->email_footer) : (isset($this->info['cc_type']) && $this->info['cc_type'] !== '' ? $this->info['cc_type'] . ' ' . $cc_num_display : '');
        ...
        // include disclaimer
        if (zen_config('EMAIL_DISCLAIMER') !== '') $email_order .= "\n-----\n" . sprintf(zen_config('EMAIL_DISCLAIMER'), zen_config('STORE_OWNER_EMAIL_ADDRESS')) . "\n\n";
        // include copyright
